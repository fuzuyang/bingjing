    ...
    try:
        ...
        for df in read_excel_in_batches(cleaned_path, BATCH_SIZE):
            ...
            if batch_num == 1 and not use_existing_mapping:
                excel_columns = list(df.columns)[1:]
            ...
            for row_idx, row in df.iterrows():
                try:
                    ...
                    if len(df.columns) > 1:
                        second_col_value = row.iloc[1] if len(row) > 1 else None
                        processed_value = process_cell_value(second_col_value)
