...
class Querykeyword:
    # 可调节的变量
    MAX_OUTPUT = 100       # 最多输出论文数量
    MIN_PAPERS = 20        # 第一部分最少输出数量
    MAX_ABSTRACT = 30      # 第二部分最多输出数量
    MIN_PAPERS_FOR_PHASE3 = 10  # 触发第三阶段的最小论文数量（新增）
    ...
    def query_papers(self, keyword_groups: List[List[str]], output_file: str = "results.txt") -> Dict[str, Any]:
        ...
        try:
            ...
            # 阶段三：如果第一、二阶段合并结果不足指定数量，使用优先级最高的一般词作为主题词
            phase31_results = []
            phase32_results = []
            phase3_used_keywords = []
            
            phase1_2_total = len(phase1_results) + len(phase2_results)
            if phase1_2_total < self.MIN_PAPERS_FOR_PHASE3 and general_words:
                print(f"第一、二阶段总计只有{phase1_2_total}篇，不足{self.MIN_PAPERS_FOR_PHASE3}篇，开始第三阶段查询...")
                
                # 使用优先级最高的一般词（第一个一般词组）作为新的主题词
                phase3_topic_synonyms = general_words[0]
                phase3_general_words = general_words[1:]  # 剩余的一般词作为新的一般词
                phase3_used_keywords = phase3_topic_synonyms
                
                if phase3_topic_synonyms:
                    print(f"第三阶段主题词（优先级最高的一般词）: {phase3_topic_synonyms}")
                    print(f"第三阶段剩余一般词: {phase3_general_words}")
                    
                    # 阶段3.1：在Paper_title和Keywords中匹配新主题词
                    print("开始阶段3.1查询：在Paper_title和Keywords中匹配新主题词...")
                    phase31_results = self._query_phase1(
                        phase3_topic_synonyms, 
                        phase3_general_words, 
                        columns
                    )
                    
                    # 阶段3.2：如果阶段3.1结果不足，在Abstract中匹配新主题词
                    if len(phase31_results) < self.MIN_PAPERS:
                        print(f"阶段3.1结果不足{self.MIN_PAPERS}篇，开始阶段3.2查询...")
                        phase32_results = self._query_phase2(
                            phase3_topic_synonyms,
                            phase3_general_words,
                            columns
                        )
            ...
            # 合并所有阶段的结果
            all_results = phase1_results + phase2_results + phase31_results + phase32_results
            
            # 去重（基于PaperID，假设每个PaperID唯一）
            unique_results = []
            seen_ids = set()
            ...
            # 限制总数
            final_results = unique_results[:self.MAX_OUTPUT]
            
            # 输出结果到文件（追加模式）
            self._write_results_to_file(
                final_results, columns, 
                phase1_results, phase2_results, 
                phase31_results, phase32_results,
                phase3_used_keywords,
                output_file, keyword_groups
            )
            ...
            return {
                ...
                "phase31_count": len(phase31_results),
                "phase32_count": len(phase32_results),
                "phase3_used_keywords": phase3_used_keywords,
                "phase1_2_total": phase1_2_total,
    ...
    def _write_results_to_file(self, results: List[Tuple], columns: List[str],
                             ...
                             phase31_results: List[Tuple], phase32_results: List[Tuple],
                             phase3_keywords: List[str],
        ...
        with open(output_file, 'a', encoding='utf-8') as f:
            ...
            if phase3_keywords:
                f.write("第三阶段检索：\n")
                f.write(f"（因第一、二阶段论文不足{self.MIN_PAPERS_FOR_PHASE3}篇，使用优先级最高的一般词作为主题词）\n")
                f.write(f"第三阶段主题词: {', '.join(phase3_keywords)}\n")
                f.write("\n")
            ...
            f.write(f"阶段3.1找到论文数：{len(phase31_results)}\n")
            f.write(f"阶段3.2找到论文数：{len(phase32_results)}\n")
            f.write(f"总计找到论文数：{len(phase1_results) + len(phase2_results) + len(phase31_results) + len(phase32_results)}\n")
            f.write(f"最终输出论文数：{len(results)}\n")
            f.write("\n")
            
            # 创建一个集合来快速检查论文是否在最终结果中
            ...
            # 处理阶段一结果
            if phase1_results:
                f.write("=== 阶段一：在标题和关键词中匹配主题词的结果 ===\n\n")
                # 筛选出在最终结果中的阶段一论文
                phase1_in_output = [r for r in phase1_results if r in results_set]
            ...
            # 处理阶段二结果
            if phase2_results:
                f.write("=== 阶段二：在摘要中检索主题词的结果 ===\n\n")
                # 筛选出在最终结果中的阶段二论文
                phase2_in_output = [r for r in phase2_results if r in results_set]
                start_idx = len(phase1_in_output) + 1
            ...
            # 处理阶段3.1结果
            if phase31_results:
                f.write("=== 阶段3.1：在标题和关键词中匹配新主题词的结果 ===\n\n")
                # 筛选出在最终结果中的阶段3.1论文
                phase31_in_output = [r for r in phase31_results if r in results_set]
                start_idx = len(phase1_in_output) + len(phase2_in_output) + 1
                for i, row in enumerate(phase31_in_output, start_idx):
                    f.write(f"【论文 {i}】\n")
                    for col, value in zip(columns, row):
                        display_value = value if value is not None else ""
                        f.write(f"{col}: {display_value}\n")
            ...
            # 处理阶段3.2结果
            if phase32_results:
                f.write("=== 阶段3.2：在摘要中检索新主题词的结果 ===\n\n")
                # 筛选出在最终结果中的阶段3.2论文
                phase32_in_output = [r for r in phase32_results if r in results_set]
                start_idx = len(phase1_in_output) + len(phase2_in_output) + len(phase31_in_output) + 1
                for i, row in enumerate(phase32_in_output, start_idx):
                    f.write(f"【论文 {i}】\n")
                    for col, value in zip(columns, row):
                        display_value = value if value is not None else ""
                        f.write(f"{col}: {display_value}\n")
    ...
    def update_config(self, max_output: int = None, min_papers: int = None, 
                     max_abstract: int = None, min_papers_for_phase3: int = None):
        """更新配置参数"""
        ...
        if min_papers_for_phase3 is not None:
            self.MIN_PAPERS_FOR_PHASE3 = min_papers_for_phase3
...
if __name__ == "__main__":
    ...
    query.update_config(
        ...
        min_papers=10, 
        max_abstract=20,
        min_papers_for_phase3=10  # 设置触发第三阶段的最小论文数
    )
    ...
    for i, keyword_groups in enumerate(batch_keyword_groups, 1):
        ...
        if "error" in result:
            ...
            # 如果有第三阶段结果，显示相关信息
            if result.get('phase31_count', 0) > 0 or result.get('phase32_count', 0) > 0:
                success_msg += f"阶段3.1找到：{result.get('phase31_count', 0)}篇\n"
                success_msg += f"阶段3.2找到：{result.get('phase32_count', 0)}篇\n"
                if result.get('phase3_used_keywords'):
                    success_msg += f"第三阶段主题词：{', '.join(result.get('phase3_used_keywords'))}\n"
            
            success_msg += f"第一、二阶段总计：{result.get('phase1_2_total', 0)}篇\n"
            ...
            all_results.append({
                ...
                "phase31_count": result.get('phase31_count', 0),
                "phase32_count": result.get('phase32_count', 0),
                "phase1_2_total": result.get('phase1_2_total', 0),
                "total_output": result.get('total_output', 0),
                "phase3_triggered": result.get('phase1_2_total', 0) < query.MIN_PAPERS_FOR_PHASE3
            })
    ...
    if successful_groups:
        ...
        total_phase31 = sum(r.get('phase31_count', 0) for r in successful_groups)
        total_phase32 = sum(r.get('phase32_count', 0) for r in successful_groups)
        total_output = sum(r.get('total_output', 0) for r in successful_groups)
        phase3_triggered_groups = sum(1 for r in successful_groups if r.get('phase3_triggered', False))
        
        print(f"\n总计统计：")
        print(f"阶段一总计找到：{total_phase1}篇")
        print(f"阶段二总计找到：{total_phase2}篇")
        print(f"阶段3.1总计找到：{total_phase31}篇")
        print(f"阶段3.2总计找到：{total_phase32}篇")
        print(f"所有组总计输出：{total_output}篇")
        print(f"触发第三阶段的组数：{phase3_triggered_groups}组")
