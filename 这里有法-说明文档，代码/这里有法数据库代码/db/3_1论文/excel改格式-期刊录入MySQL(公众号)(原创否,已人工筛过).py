    ...
    for file_path in excel_files:
        ...
        try:
            ...
            needed_columns = ['公众号', '发表期号', '文章标题', '作者', '作者单位', 
                            '内容提要', '关键词', '发布时间', '文章摘要', '期刊名称', '文章链接']  # 添加文章链接
            ...
            if '文章标题' in df.columns and '作者' in df.columns:
                def improved_extract_author(row):
                    ...
                    # 新增逻辑：如果标题中既有"|"又有冒号，优先提取"|"之后、冒号之前的部分
                    ...
                    # 如果既有竖线又有冒号，且竖线在冒号之前
                    if vertical_pos != -1 and colon_pos != -1 and vertical_pos < colon_pos:
                        extracted = title_str[vertical_pos + 1:colon_pos].strip()
                        # 清理提取的内容
                    ...
                    for pattern in patterns:
                        match = re.match(pattern, title_str)
                        if match:
            ...
            # 7. 处理内容提要和文章摘要的合并 - 在重命名之前处理
            if '文章摘要' in df.columns and '内容提要' in df.columns:
                # 如果内容提要为空，用文章摘要填充
                mask = (df['内容提要'].isna()) | (df['内容提要'].astype(str).str.strip() == '')
                if mask.sum() > 0:
                    df.loc[mask, '内容提要'] = df.loc[mask, '文章摘要']
                    print(f"  已将文章摘要合并到内容提要: {mask.sum()} 条")
            ...
            # 9. 关键修复：删除不再需要的"文章摘要"列（如果存在）
            if '文章摘要' in df.columns:
                df = df.drop(columns=['文章摘要'])
                print(f"  已删除多余的'文章摘要'列")
            
            # 10. 只删除文章标题为空的行（作者为空保留）
            before_count = len(df)
            ...
            after_count = len(df)
            print(f"  删除文章标题空值后保留: {after_count} 行 (删除: {before_count - after_count} 行)")
            
            # 统计作者为空的行数（不删除）
            if 'Author' in df.columns:
                author_null_count = df['Author'].isna().sum()
                author_blank_count = df['Author'].astype(str).str.strip().eq('').sum()
                print(f"  作者为空的行数: {author_null_count + author_blank_count} 行 (已保留)")
...
#%% 使用示例
process_folder(r"D:\这里有法_资料\已入库\3_1论文基础信息\01公众号论文-未清洗", "3_1Paper")